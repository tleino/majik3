...
create_room ()
{
  ...
  + "from falling. A make shift bridge, made of rope, goes off to "
  + "the south from here.");

  add_exit ("south", ROOM_PATH + "bridge18.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs down the "
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "oforest8.c");
   return 1;
}
}
