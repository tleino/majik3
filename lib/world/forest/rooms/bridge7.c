...
create_room ()
{
  ...
  add_exit ("west", ROOM_PATH + "bridge38.c");
  add_exit ("east", ROOM_PATH + "bridge8.c");
  add_exit ("northwest", ROOM_PATH + "bridge6.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs down the "
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "iforest9.c");
   return 1;
}
}
