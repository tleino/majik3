...
create_room ()
{
  ...
  + "Only a single ray of sunlight would be allowed through on the"
  + " sunniest day. There is no discernable path or footprints in "
  + "sight.");

  add_exit ("northeast", ROOM_PATH + "oforest10.c");
  add_exit ("southeast", ROOM_PATH + "oforest17.c");
  add_exit ("north", ROOM_PATH + "oforest9.c");
  add_exit ("east", ROOM_PATH + "oforest13.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs up the "+
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "bridge31.c");
   return 1;
}
}
