...
create_room ()
{
  ...
  + "two seperate directions from here.");

  add_exit ("west", ROOM_PATH + "bridge32.c");
  add_exit ("south", ROOM_PATH + "bridge33.c");
  add_exit ("north", ROOM_PATH + "bridge28.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs down the "
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "oforest13.c");
   return 1;
}
}
