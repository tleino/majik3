...
create_room ()
{
  ...
  add_exit ("south", ROOM_PATH + "bridge26.c");
  add_exit ("southeast", ROOM_PATH + "bridge2.c");
  add_exit ("east", ROOM_PATH + "bridge24.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs down the "
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "oforest7.c");
   return 1;
}
}
