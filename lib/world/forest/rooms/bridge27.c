...
create_room ()
{
  ...
  + "three seperate directions from here.");

  add_exit ("southwest", ROOM_PATH + "bridge30.c");
  add_exit ("south", ROOM_PATH + "bridge28.c");
  add_exit ("north", ROOM_PATH + "bridge26.c");
   ...
   else
{
   ...
   message("standard", capitalize(THIS->query_name()) + " climbs down the "
    "tree.\n", ENV(THIS), THIS);
   THIS->move_player ( ROOM_PATH + "oforest10.c");
   return 1;
}
}
