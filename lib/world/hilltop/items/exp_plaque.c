...
create_item ()
{
  ...
  set_name ("plaque");
  set_short ("a massive golden plaque \"Most Experienced\"");
  set_no_get();
...
long ()
{
  ...
  buf = lw("This ancient plaque has been shaped from a single piece of gold. "
      + "Only the most experienced mortals have their name "
      + "carved on this magical plaque, blessed by the gods.\n\n");

  buf += "^c0^cW^bb##. name            level experience^c0\n";
  k = keys(players);
  ...
  for (int i=0;i<sizeof(k);i++)
    {
      ...
      buf += sprintf ("^c0%s%2d^c0. ^c1%-15s^c0 %5d %10d\n", "^cc",
               i+1, k[i],players[k[i]][0]/1000<1?1:players[k[i]][0]/1000,players[k[i]][0]);
    }
  ...
  for (i=0;i<sizeof(u);i++)
    {
      if (!wizardp(u[i]->query_name()) && !u[i]->query_npc())
        {
          players[u[i]->query_name()] = ({ u[i]->query_experience (),
                                           time () });
        }
    }
