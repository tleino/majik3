...
create_item ()
{
  ...
  set_name ("plaque");
  set_short ("a massive golden plaque \"Top Explorers\"");
  set_no_get();
...
long ()
{
  ...
  buf = lw("This ancient plaque has been shaped from a single piece of gold. "
      + "Only the most experienced explorers have their name "
      + "carved on this magical plaque, blessed by the gods.\n\n");

  buf += "^c0^cW^bb##. name            explore  percent^c0\n";

  k = keys(players);
  k = sort_array (k, "do_sort");
  t = EXPLORE_D->query_total();
  ...
  for (int i=0;i<sizeof(k);i++)
    {
      ...
      buf += sprintf ("^c0%s%2d^c0. ^c1%-15s^c0 %7d  %7d\n", "^cc",
               i+1, k[i], players[k[i]][0], (players[k[i]][0] * 100 / t));
    }
  ...
  for (i=0;i<sizeof(u);i++)
    {
      if (!wizardp(u[i]->query_name()) && !u[i]->query_npc())
        {
          players[u[i]->query_name()] = ({ u[i]->query_explore (),
                                           time () });
        }
    }
