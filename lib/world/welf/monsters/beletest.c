...
init_monster()
{
    ...
    switch (name)
    {
		case "belemar" : call_out("do_greet", 1); break ;
		case "shinael" : call_out("do_kneel", 1); break ;
		default: command ("smile");
    }
//	message("0", "NAME: " + name + " : " + THOB->query_name() + ".\n", ENV(THOB));
}
do_greet()
{
  command("say Father!");
...
do_kneel()
{
  THOB->command ("say My lord!");
  THOB->command ("worship shinael");
}
