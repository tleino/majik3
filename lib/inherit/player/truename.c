...
int recite_true_name(string divine_chant, string target)
{
    ...
    object true_spell, target_ob;
    the_spell=handle_true_names(divine_chant);
    emote ("$n {divine} '^c1"+divine_chant+"!^c0'", THOB, 0, ENV(THOB));
    ...
    emote ("Reality ripples from the power of $s words!", THOB, 0, ENV(THOB));
    words=explode(the_spell,"-");
    test=({});
    for(i=0;i<sizeof(words);i++)
    {
    ...
    if (true_spell->query_offensive())
      {
        if (target)
          {
            target_ob = present(target);

            if (target_ob && target_ob != THOB)
              {
                target_ob->before_battle (THOB);
                THOB->set_target(target_ob->query_true_name());
                THOB->begin_battle(target_ob);
